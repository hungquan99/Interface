...
Components.Tab = (function()
	...
	function TabModule:New(Title, Icon, Parent)
		...
		function Tab:SelectSubTab(Index)
			...
			local Previous = Tab.SelectedSubTab
			Tab.SelectedSubTab = Index

			for i, SubTabObject in next, Tab.SubTabs do
				SubTabObject.Selected = (i == Index)
				SubTabObject.SetTransparency(SubTabObject.Selected and 0.85 or 1)
		...
		function Tab:AddSubTab(Title, Icon)
			...
			if not fischbypass then
				if Library:GetIcon(SubTabIcon) then
					SubTabIcon = Library:GetIcon(SubTabIcon)
				end
			...
			SubTab.Button = New("TextButton", {
				...
				New("TextLabel", {
					Name = "Label",
					AnchorPoint = Vector2.new(0, 0.5),
					Position = SubTabIcon and UDim2.new(0, 20, 0.5, 0) or UDim2.new(0, 0, 0.5, 0),
					Text = Title,
					RichText = true,
					FontFace = Font.new(
						"rbxasset://fonts/families/GothamSSm.json",
				...
				New("ImageLabel", {
					...
					BackgroundTransparency = 1,
					Image = SubTabIcon and SubTabIcon or nil,
					ThemeTag = {
						ImageColor3 = "Text",
					},
				}),
			...
			SubTab.UnderlineMotor, SubTab.SetUnderline = Creator.SpringMotor(1, SubTab.Button.Underline, "BackgroundTransparency")

			Creator.AddSignal(SubTab.Button.MouseEnter, function()
				SubTab.SetTransparency(SubTab.Selected and 0.8 or 0.93)
			end)
			Creator.AddSignal(SubTab.Button.MouseLeave, function()
				SubTab.SetTransparency(SubTab.Selected and 0.85 or 1)
			end)
			Creator.AddSignal(SubTab.Button.MouseButton1Down, function()
				SubTab.SetTransparency(0.7)
			end)
	...
	function TabModule:SelectTab(Tab)
		...
		-- Token guards against overlapping/late transitions when tabs are
		-- clicked rapidly in succession, which previously caused flicker.
		TabModule.SwitchToken = TabModule.SwitchToken + 1
		local Token = TabModule.SwitchToken
		...
		task.spawn(function()
			...
			task.wait(0.12)
			if TabModule.SwitchToken ~= Token then
				return
			end
