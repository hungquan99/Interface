...
if succ then
  pcall(function()
    Players.LocalPlayer:Kick("[hungquan99]: Attempt to tamper by replacing the clone function!")
    task.wait(1)
  end)
  while true do end
end
...
if getgenv() == getfenv(0) then
  pc(function()
    Players.LocalPlayer:Kick("[hungquan99]: Attempt to tamper by doing an enviroment logger")
    task.wait(1)
  end)
  CrashUser()
end
...
local isHooked = function(func, name)
  ...
  if succ and result == true then
    pc(function()
      Players.LocalPlayer:Kick("[hungquan99]: Attempt to tamper by hooking the " .. name .." function!")
      task.wait(1)
    end)
    CrashUser()
  end
...
local isReplaced = function(func, name)
  ...
  if succ then
    pc(function()
      Players.LocalPlayer:Kick("[hungquan99]: Attempt to tamper by replacing the " .. name .." function!")
      task.wait(1)
    end)
    CrashUser()
  end
...
if ishooked(print) == false then
  pcall(function()
    Players.LocalPlayer:Kick("[hungquan99]: Attempt to return a false value, 1")
    task.wait()
  end)
else
  succ, er = pc(function()
...
if ishooked(warn) == false then
  pcall(function()
    Players.LocalPlayer:Kick("[hungquan99]: Attempt to return a false value, 2")
    task.wait()
  end)
else
  succ, er = pc(function()
...
if ishooked(error) == false then
  pcall(function()
    Players.LocalPlayer:Kick("[hungquan99]: Attempt to return a false value, 3")
    task.wait()
  end)
else
  succ, er = pc(function()
...
local function checkTextObject(object)
	...
	if success and typeof(text) == "string" then
		...
		if s_find(lowerText, blockedString, 1, true) then
			object.Text = "[hungquan99]: Hmh, attempt to display a URL. Skid"
			return
		end
		for _, blocked in ipairs(blockedTStrings) do
			if string.find(lowerText, blocked, 1, true) then
				object.Text = "[hungquan99]: Hmh,blocked content detected."
				return
			end
		end
	end
	...
	task.spawn(function()
		while runAntiHook == true do
			...
			if isSecure() == false then
				pc(function()
					Players.LocalPlayer:Kick("[hungquan99]: Detected some Critical functions were hooked!")
					task.wait(1)
				end)
				CrashUser()
			end
				...
				pc(function()
					for i, v in ipairs(HttpFunctions) do
						...
						if succ and res == true then
							...
							pc(function()
								Players.LocalPlayer:Kick("[hungquan99]: Detected some HTTP/s functions were hooked!")
								task.wait(1)
							end)
							CrashUser()
						end
...
if c == 1 then
  pc(function()
    Players.LocalPlayer:Kick("[hungquan99]: Integrity check failed, 1")
		task.wait(1)
	end)
	CrashUser()
end
...
if c == 2 then
  pc(function()
    Players.LocalPlayer:Kick("[hungquan99]: Integrity check failed, 2")
		task.wait(1)
	end)
	CrashUser()
end
...
if ishooked and ih(delfile) == true then
  pc(function()
    Players.LocalPlayer:Kick("[hungquan99]: Attempt to tamper AHS with a hooked delfile")
		task.wait(1)
	end)
	CrashUser()
end
...
if ishooked and ih(hookfunction) == true then
  pc(function()
    Players.LocalPlayer:Kick("[hungquan99]: Attempt to tamper AHS with a hooked hookfunction!")
		task.wait(1)
	end)
	CrashUser()
end
...
if not Status1 == true then
  if ishooked and ih(writefile) == true then
    pc(function()
      Players.LocalPlayer:Kick("[hungquan99]: Attempt to tamper AHS with a hooked writefile, " .. t_string(Status1))
      task.wait(1)
    end)
    while true do 
      print("[hungquan99]: Attempt to tamper with AHS with a hooked writefile, " .. t_string(Status1))
      task.wait(1) 
    end
  end
end
...
if not Status2 == true then
  if ishooked and ih(appendfile) == true then
    pc(function()
      Players.LocalPlayer:Kick("[hungquan99]: Attempt to tamper AHS with a hooked appendfile, " .. Status2)
      task.wait(1)
    end)
    while true do 
      print("[hungquan99]: Attempt to tamper with AHS with a hooked appendfile, " .. Status2)
      task.wait(1) 
    end
  end
end
      ...
      old3 = hf(setclipboard, newcclosure(function(content, Auth)
        ...
        for _, Keyword in ipairs(BannedKeywords) do
          ...
          if s_find(content, v) or string.match(content, v) then
            content = "[hungquan99]: Attempt to setclipboard a protected URL isnt alloerd"
          end
        end
        return old3(content)
      end))
...
old = hf(writefile, newcclosure(function(FileName, FileContent, Auth)
  ...
  for _, Keyword in ipairs(BannedKeywords) do
    ...
    if s_find(content, v) or string.match(content, v) then
      FileContent = "return print('[hungquan99]: Attempt to retrive a protected URL!')"
    end
  end
...
if writefile_num == 0 then
  pc(function()
    Players.LocalPlayer:Kick("[hungquan99]: Attempt to tamper using hookfunction!")
		task.wait(1)
	end)
	CrashUser()
end
...
pc(function()
  loadstring(setmetatable({}, {
    __tostring = function()
      return "[hungquan99]: Trying to see the contents of loadstring, nice try!"
    end
  }))
end)
