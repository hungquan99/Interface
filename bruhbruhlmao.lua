local function kick(reason)
    game.Players.LocalPlayer:Kick(reason or "Blah Blah")
end
...
if detectedExecutor:lower():find("xeno") or detectedExecutor:lower():find("solara") or detectedExecutor:lower():find("nezur") then
	...
	if version == "1.5" then
		...
		local Overflow = function(Function)
		for Index = 1, 197 do
		...
		local succ, info = pcall(Overflow(request))
		if succ == false and info == "C stack overflow" then
			-- hooked or tampered
		else
			-- what to do if not hooked or tampered
		end
		...
		return "Custom Secure - v1.5", "Custom"
		end
	...
	elseif version == "1.0" then
		...
		hq99.getVersion = function()
		return "Custom Secure - v1.0", "Custom"
		end
